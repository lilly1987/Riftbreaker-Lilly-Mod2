...
function dom_mananger:Update()
	...
	LogService:DebugText( 50, 100, debug);
end
...
function dom_mananger:LogicEntryTableCheck( logicTable, logString, failedLogicFileTable )
	...
	if ( logicTable ~= nil ) then
		for data in Iter( logicTable ) do 
			...
			if ( ResourceManager:ResourceExists( "FlowGraphTemplate", data.entryLogic ) ) then
				log = log .. " EXIST"
			else
				table.insert( failedLogicFile, log )
				log = log .. " NOT EXIST"
		...
		for data in Iter( logicTable ) do 
			...
			if ( ResourceManager:ResourceExists( "FlowGraphTemplate", data.exitLogic ) ) then
				log = log .. " EXIST"
			else
				table.insert( failedLogicFile, log )
				log = log .. " NOT EXIST"
...
function dom_mananger:OnLuaGlobalEvent( evt )
	...
	LogService:Log( "dom_mananger:OnLuaGlobalEvent : event - " .. eventName )
